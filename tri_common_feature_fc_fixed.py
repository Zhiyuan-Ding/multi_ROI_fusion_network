...
def basic_combination(model,transfered_size):
...
class Main_Net(nn.Module):
    def __init__(self,feature_extraction_layer,branch1,branch2,branch3):
        ...
        self.fcn1=nn.Sequential(nn.Linear(in_features=32*32*32, out_features=2, bias=True)
#                                nn.Sigmoid(),
...
#    os.environ['CUDA_VISIBLE_DEVICES']='4,5'
    ...
    exp_type='tri_kp_common_with_fixed_fcn_grid_'
...
def main_worker(local_rank,nprocs, args):
    ...
    args.saving_full_process_flag=-1
    ...
    grid_full_process_mat=np.zeros([5,7,args.full_epoch,14])
...
#        set_gradient(fc3,n_layers=2,state=False)
        ...
        current_model=Main_Net(feature_extraction_layer,fc1,fc2,fc3)
        ...
        best_acc_main= .0
        criterion = nn.CrossEntropyLoss().cuda(local_rank)
        weight_decay_mat=[1e-2,1e-4,1e-6,1e-8]
        ...
        n_decay=-1
        for weight_decay_element in weight_decay_mat:
            n_decay=n_decay+1
            
            optimizer = torch.optim.SGD(filter(lambda p: p.requires_grad, model_para.parameters()), args.lr, momentum=args.momentum, weight_decay=1e-4)
            
     
            args.net_module_flag=1
            args.saving_full_process_flag=1
            for epoch in range(args.full_epoch):
                
                train_sampler.set_epoch(epoch)
                test_sampler.set_epoch(epoch)
                
                adjust_learning_rate(optimizer, epoch, args, tune_epoch=30)
                
                acc_train_main,train_loss=train(train_loader, model_para, criterion, optimizer, epoch, local_rank, args)
                grid_full_process_mat[n_split,n_decay,epoch,0]=acc_train_main.Accuracy
                grid_full_process_mat[n_split,n_decay,epoch,1]=acc_train_main.TPR
                grid_full_process_mat[n_split,n_decay,epoch,2]=acc_train_main.FPR
                grid_full_process_mat[n_split,n_decay,epoch,3]=acc_train_main.Precision
                grid_full_process_mat[n_split,n_decay,epoch,4]=acc_train_main.F1score
                grid_full_process_mat[n_split,n_decay,epoch,5]=acc_train_main.Kappa
                grid_full_process_mat[n_split,n_decay,epoch,6]=train_loss
                acc_main, test_loss = validate(test_loader, model_para, criterion, local_rank, args)
                grid_full_process_mat[n_split,n_decay,epoch,7]=acc_main.Accuracy
                grid_full_process_mat[n_split,n_decay,epoch,8]=acc_main.TPR
                grid_full_process_mat[n_split,n_decay,epoch,9]=acc_main.FPR
                grid_full_process_mat[n_split,n_decay,epoch,10]=acc_main.Precision
                grid_full_process_mat[n_split,n_decay,epoch,11]=acc_main.F1score
                grid_full_process_mat[n_split,n_decay,epoch,12]=acc_main.Kappa
                grid_full_process_mat[n_split,n_decay,epoch,13]=test_loss               
        ...
        full_process_path=args.save_path_main+'full_process'+str(n_split)+'time.npy'
        np.save(full_process_path,grid_full_process_mat)        
    ...
    if args.net_module_flag==-1:
        ...
        for i,(data,label) in enumerate(train_loader):
             ...
             if i % args.print_freq == 0:
                ...
                progress.display(i)
             if args.saving_full_process_flag==1 :
                 return result,reduced_loss
...
def adjust_learning_rate(optimizer, epoch, args, tune_epoch=30):
    """Sets the learning rate to the initial LR decayed by 10 every 30 epochs"""
    lr = args.lr * (0.1**(epoch // tune_epoch))
