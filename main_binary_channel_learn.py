# -*- coding: utf-8 -*-
"""
Created on Thu Mar  4 11:15:49 2021
...
parser.add_argument('-a','--arch',default='resnet18',
                    help='model name')
parser.add_argument('--tf_epochs', default=200, type=int,
                     help='training epochs for transfer learning epochs')
...
def model_reshape(model,original_size, transfered_size):
    x=original_size[0]/transfered_size[0]
    y=original_size[1]/transfered_size[1]
    z=original_size[2]/transfered_size[2]
    reshaped_model=nn.Sequential(nn.Upsample(scale_factor=(x,y,z)),
                                 model)
...
def load_data(data_list):
    ...
    for i in range(n_sample):
        ...
        exec(f'data%s=np.load(data_list[{i}])' %i )
        exec(f'data.append(data%s)'%i)
...
def main():
    ...
    data_list=['D:\\temp_python\\Hippocampus_LAD_NC.npy','D:\\temp_python\\Hippocampus_R_data.npy']
    [data0,data1]=load_data(data_list)
#    data_path='/home/ding/exp_2/data/Hippocampus_LAD_NC.npy'
#    data=np.load(data_path)
#    data_after_transfer_path='/home/ding/exp_2/data/Hippocampus_R_data.npy'
#    data_after_transfer=np.load(data_after_transfer_path)
...
#    data=InputDataset(data,label)
    data0=InputDataset(data0,label)
    data1=InputDataset(data1,label)
    
    args.data0=data0
    args.data1=data1
    
#    args.data_after_transfer=data_after_transfer
    
    args.arch='resnet10'
    
    model1=resnet3d.ResNet(resnet3d.Bottleneck, [1, 1, 1, 1],resnet3d.get_inplanes(),
                         n_input_channels=1,
                         conv1_t_size=27,
                         conv1_t_size2=27,
                         conv1_t_size3=20,
    ...
    model2=resnet3d.ResNet(resnet3d.Bottleneck, [1, 1, 1, 1],resnet3d.get_inplanes(),
                     n_input_channels=1,
                     conv1_t_size=27,
                     conv1_t_size2=27,
                     conv1_t_size3=20,
    ...
    reshape_model=model_reshape(model,data_after_transfer.shape[1:],data_after_transfer.shape[1:])



    mp.spawn(main_worker, nprocs=args.nprocs, args=(args.nprocs, args, reshape_model))
...
def main_worker(local_rank,nprocs, args, model):
    ...
    for train_idx, test_idx in kf.split(args.data.Data,args.label):
        ...
        model_para = torch.nn.parallel.DistributedDataParallel(model, device_ids=[local_rank],find_unused_parameters=True)
        best_acc1= .0
        best_acc1_tf=.0
        ...
        train_loader,test_loader,train_sampler,test_sampler= dataloader_preprocessing(train_idx,test_idx,args,args.data.Data)     
        ...
        result_path='/home/ding/exp_2/result/result_Hippocampus_L_'+args.arch+'_'+str(n_split)+'time.npy'
        ...
        set_gradient(model_para)
        
        train_loader_tf,test_loader_tf,train_sampler_tf,test_sampler_tf= dataloader_preprocessing(train_idx,test_idx,args,args.data_after_transfer)
        
        optimizer_tf = torch.optim.SGD(filter(lambda p: p.requires_grad, model_para.parameters()), args.lr, momentum=args.momentum, weight_decay=args.weight_decay)
        
        for epoch in range(args.tf_epochs):
            
            train_sampler_tf.set_epoch(epoch)
            test_sampler_tf.set_epoch(epoch)
            
            adjust_learning_rate(optimizer_tf, epoch, args)
        
            train(train_loader_tf, model_para, criterion, optimizer_tf, epoch, local_rank, args)
            
            acc1_tf = validate(test_loader_tf, model_para, criterion, local_rank, args)
            
            is_best_tf = acc1_tf.Accuracy > best_acc1_tf
            best_acc1_tf = max(acc1_tf.Accuracy, best_acc1_tf)
                ...
                save_checkpoint(
                    {
                        ...
                        'arch': args.arch,
                        'state_dict': model_para.module.state_dict(),
                        'best_acc1_tf': best_acc1_tf,
                    }, is_best_tf)
        
        result_path_tf='/home/ding/exp_2/result/result_Hippocampus_R_'+args.arch+'_after_transform_'+str(n_split)+'time.npy'
        result_mat_tf[n_split,0]=acc1_tf.Accuracy
        result_mat_tf[n_split,1]=acc1_tf.TPR
        result_mat_tf[n_split,2]=acc1_tf.FPR
        result_mat_tf[n_split,3]=acc1_tf.Precision
        result_mat_tf[n_split,4]=acc1_tf.F1score
        result_mat_tf[n_split,5]=acc1_tf.Kappa
        
        n_split=n_split+1
        np.save(result_path_tf,result_mat_tf)
