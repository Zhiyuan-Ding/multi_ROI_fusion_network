# -*- coding: utf-8 -*-
"""
Created on Wed Mar 31 21:13:21 2021
...
# -*- coding: utf-8 -*-
"""
Created on Wed Mar 17 09:57:59 2021
...
# -*- coding: utf-8 -*-
"""
Created on Sun Mar 14 10:08:21 2021
...
#settings
parser = argparse.ArgumentParser(description='tribal channel brain region model')
...
parser.add_argument('-a','--arch',default='resnet10',
                    help='model name')
...
def kronecker_product(x1,x2,x3):
    ...
    for i in range (n_sample):
        ...
        temp_feature=(current_feature1*current_feature2).view(1,size1*size2)

        current_feature=(temp_feature*current_feature3).view(-1)

        kron_prod.append(current_feature)
    kron_prod=torch.cat(kron_prod, dim=0)
    output=kron_prod.view(n_sample,size1*size2*size3)
    return output
    


def feature_kron(feature1,feature2,feature3):
    assert feature1.shape[0] == feature2.shape[0]==feature3.shape[0]
    feature_kron=kronecker_product(feature1,feature2,feature3)
...
class Main_Net(nn.Module):
    def __init__(self,branch1,branch2,branch3, compressed1, compressed2,compressed3):
        ...
        self.compressed3=compressed3
        
        self.fcn1=nn.Sequential(nn.Linear(in_features=16*16*16, out_features=2, bias=True)
                                )
...
#        feature2=feature2.view(n_sample,-1)
        ...
        feature3=self.compressed3(feature3)
        
        feature=feature_kron(feature1,feature2,feature3)
...
#    args.nprocs = torch.cuda.device_count()
    ...
    os.environ['MASTER_ADDR'] ='10.1.1.122'
    os.environ['MASTER_PORT'] = '8888'
    
    data_path1='/share22/home/lidingli/chz/dzy/data/Hippocampus_LAD_NC.npy'
    data_path2='/share22/home/lidingli/chz/dzy/data/Hippocampus_R_data.npy'
    data_path3='/share22/home/lidingli/chz/dzy/data/ParaHippocampus_L_data.npy'
    ...
    model3=resnet3d.ResNet(resnet3d.BasicBlock, [1, 1, 1, 1],resnet3d.get_inplanes(),
                     ...
                     shortcut_type='B',
                     n_classes=2)    
    args.save_path1='/share22/home/lidingli/chz/dzy/result/result_Hippocampus_L_kronecker_tri_exp1_'
    args.save_path2='/share22/home/lidingli/chz/dzy/result/result/result_Hippocampus_R_kronecker_tri_exp1_'
    args.save_path3='/share22/home/lidingli/chz/dzy/result/result_ParaHippocampus_L_kronecker_tri_exp1_'
    
    
    args.orginal_params_save_path1='/share22/home/lidingli/chz/dzy/result/Hippocampus_L_original_tri_exp1_'
    args.orginal_params_save_path2='/share22/home/lidingli/chz/dzy/result/Hippocampus_R_original_tri_exp1_'
    args.orginal_params_save_path3='/share22/home/lidingli/chz/dzy/result/ParaHippocampus_L_tri_exp1_'
    
    
    args.params_save_path1='/share22/home/lidingli/chz/dzy/result/Hippocampus_L_kronecker_tri_exp1_'
    args.params_save_path2='/share22/home/lidingli/chz/dzy/result/Hippocampus_R_kronecker_tri_exp1_'
    args.params_save_path3='/share22/home/lidingli/chz/dzy/result/ParaHippocampus_L_kronecker_tri_exp1_'
    
    args.model1=model1
    args.model2=model2
    args.model3=model3
    
    args.save_path_main='/share22/home/lidingli/chz/dzy/result/result_kronecker_product_kronecker_tri_exp1_'
...
def main_worker(local_rank,nprocs, args):
    ...
    original_params3=args.model3.state_dict()
...
#        torch.save(current_model.state_dict(), params_save_path)
        ...
        current_model3=args.model3
        current_model3.load_state_dict(original_params3)
        current_model3.cuda(local_rank)
        model_para = torch.nn.parallel.DistributedDataParallel(current_model3, device_ids=[local_rank])
        best_acc3= .0
        criterion = nn.CrossEntropyLoss().cuda(local_rank)
        optimizer = torch.optim.SGD(model_para.parameters(), args.lr, momentum=args.momentum, weight_decay=args.weight_decay)
        ...
        set_gradient(current_model3)
        model_channel3= nn.Sequential(*list(current_model3.children())[:-1])        
        
        
        criterion = nn.MSELoss().cuda(local_rank)
        compressed_channel1=nn.Sequential(
                                    Compressed_Net(512,16),
                                    Uncompressed_Net(16,512))
        ...
        criterion = nn.MSELoss().cuda(local_rank)
        compressed_channel3=nn.Sequential(
                                    Compressed_Net(512,16),
                                    Uncompressed_Net(16,512))
        compressed_channel3.cuda(local_rank)
        model_channel3.cuda(local_rank)
        model_para = torch.nn.parallel.DistributedDataParallel(compressed_channel3, device_ids=[local_rank])
        model_para3 = torch.nn.parallel.DistributedDataParallel(model_channel3, device_ids=[local_rank])
        train_loader,test_loader,train_sampler,test_sampler= dataloader_preprocessing(train_idx,test_idx,args,args.data3)     
        optimizer = torch.optim.SGD(filter(lambda p: p.requires_grad, model_para.parameters()), args.lr, momentum=args.momentum, weight_decay=args.weight_decay)
        ...
        for epoch in range(args.epochs):
            ...
            train_compressed(train_loader, model_para, model_para3, criterion, optimizer, epoch, local_rank, args)
        ...
        compress_net3=nn.Sequential(*list(compressed_channel3.children())[:-1])
        
        set_gradient(compress_net1, n_layers=0)
        set_gradient(compress_net2, n_layers=0)
        set_gradient(compress_net2, n_layers=0)
        current_model=Main_Net(model_channel1,model_channel2,model_channel3, compress_net1,compress_net2,compress_net3)
        ...
        criterion = nn.CrossEntropyLoss().cuda(local_rank)
        optimizer = torch.optim.SGD(filter(lambda p: p.requires_grad, model_para.parameters()), args.lr, momentum=args.momentum, weight_decay=args.weight_decay)
        flatten_data=data_synthesize(args.data1,args.data2,args.data3)
