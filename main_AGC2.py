...
class AGC_ResNet(nn.Module):
    def __init__(self,ResNet_Model, AGC):
        super(AGC_ResNet,self).__init__()
        
        self.block1=nn.Sequential(*list(ResNet_Model.children())[-10:-6])
        self.attention1 = AGC(in_channel=64, out_channel=64, hidden_channel=64)
        self.block2=nn.Sequential(*list(ResNet_Model.children())[-6:-5])
        self.attention2 = AGC(in_channel=64, out_channel=64, hidden_channel=64)
        self.block3=nn.Sequential(*list(ResNet_Model.children())[-5:-4])
        self.attention3 = AGC(in_channel=128, out_channel=128, hidden_channel=128)
        self.block4=nn.Sequential(*list(ResNet_Model.children())[-4:-3])
        self.attention4 = AGC(in_channel=256, out_channel=256, hidden_channel=256)
        self.block5=nn.Sequential(*list(ResNet_Model.children())[-3:-2])
        self.attention5 = AGC(in_channel=512, out_channel=512, hidden_channel=512)
...
#    os.environ['CUDA_VISIBLE_DEVICES']='4,5'
    ...
    data_path='/home/ding/exp_2/data/Hippocampus_R_data.npy'
    ...
    total_model=AGC_ResNet(model,AGC)
...
def main_worker(local_rank,nprocs, args, model):
    ...
    for train_idx, test_idx in kf.split(args.data.Data,args.label):
        ...
        result_path='/home/ding/exp_2/result/result_Hippocampus_R_AGC_ResNet_'+str(n_split)+'time.npy'
